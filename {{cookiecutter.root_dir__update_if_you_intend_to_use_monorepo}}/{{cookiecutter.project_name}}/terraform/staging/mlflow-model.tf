...
* [Recurring model training job]({{cookiecutter.databricks_staging_workspace_host}}#job/${databricks_job.model_training_job.id}): trains fresh model versions using the latest ML code.
...
* [Recurring batch inference job]({{cookiecutter.databricks_staging_workspace_host}}#job/${databricks_job.batch_inference_job.id}): applies the latest ${local.env} model version for batch inference.
EOF
}
