...
# ``{{template "project_name_alphanumeric_underscore"}}/databricks-resources/model-workflow-resource.yml``
#
...
if config['experiment']['name'].startswith(f"/{env}-{{cookiecutter.experiment_base_name}}"):
    print("WARNING: The experiment name may not have been set correctly. Please confirm that the experiment name in the profile YAML file matches the experiment_name variable in {{template "project_name_alphanumeric_underscore"}}/bundle.yml.")
